...
if (isset($_POST['reply_msg'])){
    ...
    $query = "insert into class_general (general_message, general_file, to_id, from_id, opened, recipient_delete, sender_delete, time_sent, class_id,general_reply,general_code) values ('$message','$msg_file','$recipient','$from_id','0','0','0', NOW(),'$class_id','1','$general_code')";
...
/*PHP for Pagination -Jaycode*/
  ...
  $msg_query_select =" select * from class_general where (class_id = '{$_SESSION['current_class']}' and from_id ='{$_SESSION['current_member']}' and sender_delete='0') or (class_id = '{$_SESSION['current_class']}' and to_id ='{$_SESSION['current_member']}' and recipient_delete='0' ) group by general_message ";
  $find_count = mysqli_query($connection, $msg_query_select);
  $msg_count = mysqli_num_rows($find_count);
  $msg_count =ceil($msg_count / 5 ) ;
...
$msg_query = "select general_message,to_id,from_id,general_file,general_reply,general_id,time_sent,sender_delete,general_code,opened,general_cat  from class_general where (class_id = '{$_SESSION['current_class']}' and from_id ='{$_SESSION['current_member']}' and sender_delete='0') or (class_id = '{$_SESSION['current_class']}' and to_id ='{$_SESSION['current_member']}' and recipient_delete='0' ) group by general_message  order by general_id  desc limit $page_1, 6 ";
  ...
  <form id='message_list' action='index.php' method='post' enctype='multipart/form-data'>
   ";
   ...
   $style='';
    ...
    $msg_photo ='';
    if ($general_cat == 'general'){

        $msg_photo = 'msg.png';
   }else if ($general_cat =='reminder'){
     $msg_photo = 'reminder.png';

   }else if ($general_cat == 'homework'){

         $msg_photo = 'homework.png';
   }
   else{
     $msg_photo = 'msg.png';
   }
   if ($opened==0){
        $style = "style='background-color:rgb(30, 138, 23);'";
            ...
            <img class='msglogo' src='../images/$msg_photo'>
                ...
                <div class='panel-body'>
                    ...
                    <button name='delete_msg1' value='$general_id' class='btn btn-danger'>Delete</button>
                        <div class='collapse indent indentright' id='reply$count'> 
                            ...
                            <textarea  rows='8' placeholder='Enter Message (max 1000 characters)' name='message' required></textarea>
